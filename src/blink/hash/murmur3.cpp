...
// BLINK_BEGIN
// Modified to compile inside the blink lib.
// BLINK_END
...
// BLINK_BEGIN
//------------------------------------------------------------------------------
uint32_t bl_hash_murmur3(const void* __restrict buf, uint32_t len, uint32_t seed) {
  uint32_t result;
  MurmurHash3_x86_32(buf, len, seed, &result);
  return result;
}
// BLINK_END
